...
def generate_heightmap(
    size: int, octaves: int, scale: float = 100.0, seed: int | None = None
) -> np.ndarray:
    logger.debug(f"Generating heightmap: size={size}, octaves={octaves}, seed={seed}")
...
def render_map_to_png(
    size: int, octaves: int, filename: str, seed: int | None = None
) -> None:
    logger.info(
        f"Rendering map to file: {filename} (size={size}, octaves={octaves}, "
        f"seed={seed})"
    )
    heightmap = generate_heightmap(size, octaves, scale=size / 4, seed=seed)
...
def render_map_to_buffer(
    size: int, octaves: int, seed: int | None = None
) -> io.BytesIO:
    logger.info(
        f"Rendering map to buffer (size={size}, octaves={octaves}, seed={seed})"
    )
    heightmap = generate_heightmap(size, octaves, scale=size / 4, seed=seed)
    fig = create_figure(heightmap)
    output = io.BytesIO()
